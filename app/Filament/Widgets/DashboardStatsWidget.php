...
class DashboardStatsWidget extends BaseWidget
{
    ...
    protected function getStats(): array
    {
        ...
        // Calculate Grand Total Revenue from Token Purchases
        // Course enrollments no longer generate direct revenue (they burn tokens)
        ...
        for ($i = 3; $i >= 0; $i--) {
            $date = now()->subDays($i);
            // Course enrollments no longer generate IDR, they burn tokens
        ...
        if (auth()->user()->isSuperAdmin()) {
            $stats[] = Stat::make('Grand Total Revenue', 'Rp ' . number_format($grandTotalRevenue, 0, ',', '.'))
